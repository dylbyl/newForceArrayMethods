const businesses = [
  ...
  document.querySelector("#business-container").innerHTML += "<h1>Businesses</h1>"

  //forEach: like a for loop, but iterates on each item of the array automatically, without an iterator variable
  businesses.forEach(business => {
    document.querySelector("#business-container").innerHTML += `
  ...
  const newYorkBusinessArray = businesses.filter(business => business.addressStateCode === "NY");

  //Filters items into a new array according to a search value we specify (here, it's looking for a certain companyIndustry)
  const manufacturingBusinessArray = businesses.filter(business => business.companyIndustry==="Manufacturing")
  console.log(manufacturingBusinessArray)


  manufacturingBusinessArray.forEach(business => {
    document.querySelector("#business-container").innerHTML += `
    <h2>${business.companyName}</h2>
    <p>${business.addressFullStreet}</p>
    <p>${business.addressCity}, ${business.addressStateCode} ${business.addressZipCode}</p>
  ...
  //Takes very specific properties out of the array and maps them to a new array
  const mapArray = businesses.map(business => {
      return {
...
const totalRainfall = monthlyRainfall.reduce((addedTotal, singleRainfall) => addedTotal += singleRainfall)
console.log(totalRainfall)

const words = ["The", "quick", "brown", "fox", "jumped", "over", "the", "lazy", "dog"]
const sentence = words.reduce((fullSentence, singleWord) => fullSentence += `${singleWord} `, "")
console.log(sentence)

const alphabetArray = words.sort((a, b) => a.toLowerCase() > b.toLowerCase() ? 1 : -1)
console.log(alphabetArray)

const newAlphabetArray = businesses.sort((a, b) => a.companyName.toLowerCase() > b.companyName.toLowerCase() ? 1 : -1)
...
planetEl.innerHTML += `<h3>----here are the planets----</h3>`
...
*/
 const capitalPlanetArray = planets.map(planet =>{
    return planet.charAt(0).toUpperCase() + planet.slice(1);
 })

 capitalPlanetArray.forEach(planet => {
    planetEl.innerHTML += `<h3>${planet}</h3>`
})
planetEl.innerHTML += `<h3>----below are the E planets----</h3>`
...
const ePlanetArray = capitalPlanetArray.filter(planet => planet.includes("e"))

ePlanetArray.forEach(planet => {
    planetEl.innerHTML += `<h3>${planet}</h3>`
})
...
const customerEmails = customers.map(customer => customer.contacts.email)
const flatCustomerEmails = customerEmails.flat();
console.log(flatCustomerEmails)

document.getElementById("spam-emails").innerHTML += "<h4>----here are the spam emails----</h4>"

flatCustomerEmails.forEach(email => {
document.getElementById("spam-emails").innerHTML += `<p>${email}</p>`
})
...
const finalIntegers = integers.sort((a, b) => a < b ? 1 : -1).filter(number => number<=19).map(number => {
    return number * 1.5
})
